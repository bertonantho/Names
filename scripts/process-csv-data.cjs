...
async function processCSVData() {
  ...
  try {
    ...
    const processedNames = Array.from(nameStats.values())
      .map((name) => ({
        name: name.name,
        sex: name.sex,
    ...
    const summary = {
      ...
      totalBirths: Array.from(nameStats.values()).reduce(
        (sum, name) => sum + name.totalCount,
        0
      ),
      yearRange: {
    ...
    console.log(
      `- Year range: ${summary.yearRange.min} - ${summary.yearRange.max}`
    );
