...
async function splitJsonData() {
  ...
  if (!fs.existsSync(inputPath)) {
    console.error(
      'Source file processed_names.json not found. Please run "npm run process-csv" first or ensure split files are committed.'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `Total file size: ${(fs.statSync(inputPath).size / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    console.log(
      `✅ Summary saved: ${(fs.statSync(summaryPath).size / 1024).toFixed(0)} KB`
    );
    ...
    console.log(
      `✅ Boys names saved: ${(fs.statSync(boysPath).size / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `✅ Girls names saved: ${(fs.statSync(girlsPath).size / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    recentYears.forEach((year) => {
      ...
      const topBoys = boysNames
        .filter(
          (name) => name.yearlyData[yearKey] && name.yearlyData[yearKey] > 0
        )
        .sort(
          (a, b) => (b.yearlyData[yearKey] || 0) - (a.yearlyData[yearKey] || 0)
        )
        .slice(0, 50)
        .map((name) => ({
          name: name.name,
    ...
    console.log(
      `✅ Popular names by year saved: ${(fs.statSync(popularPath).size / 1024).toFixed(0)} KB`
    );
    ...
    [boysNames, girlsNames].forEach((namesList, index) => {
      ...
      const trending = namesList
        .map((name) => {
          ...
          const growthRate =
            avgOlder > 0 ? (count2024 - avgOlder) / avgOlder : 0;
    ...
    console.log(
      `✅ Trending names saved: ${(fs.statSync(trendingPath).size / 1024).toFixed(0)} KB`
    );
    ...
    console.log(
      `✅ Search index saved: ${(fs.statSync(searchIndexPath).size / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    for (let i = 0; i < boysNames.length; i += CHUNK_SIZE) {
      ...
      const chunkPath = path.join(
        outputDir,
        `boys_chunk_${Math.floor(i / CHUNK_SIZE)}.json`
      );
    ...
    for (let i = 0; i < girlsNames.length; i += CHUNK_SIZE) {
      ...
      const chunkPath = path.join(
        outputDir,
        `girls_chunk_${Math.floor(i / CHUNK_SIZE)}.json`
      );
      fs.writeFileSync(chunkPath, JSON.stringify(chunk, null, 2));
    }
    ...
    console.log(
      `✅ Created ${boysChunks} boys chunks and ${girlsChunks} girls chunks`
    );
    ...
    console.log(
      `✅ Manifest saved: ${(fs.statSync(manifestPath).size / 1024).toFixed(0)} KB`
    );

    // Summary
    console.log('\n📊 Splitting Summary:');
    console.log(
      `Original file: ${(fs.statSync(inputPath).size / 1024 / 1024).toFixed(2)} MB`
    );

    const totalSplit = fs
      .readdirSync(outputDir)
      .filter(
        (file) => file.endsWith('.json') && file !== 'processed_names.json'
      )
      .reduce((total, file) => {
    ...
    console.log(
      `Split files total: ${(totalSplit / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `Compression achieved: ${(((fs.statSync(inputPath).size - totalSplit) / fs.statSync(inputPath).size) * 100).toFixed(1)}%`
    );
